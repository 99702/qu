...
public class PostController {
    ...
    public List<GetAPostDTO> getAllPost(@RequestParam(value = "pageNo", required = false) Integer pageNo) {
    ...
    public List<GetAPostDTO> getByPostDescription(@RequestParam("description") String desc) {
        return postService.fetchPostByDescription(desc);
    }

    @PostMapping(PathConstant.SEARCH_POST_BY + "/u")
    public List<GetAPostDTO> getCurrentUserPost(@RequestParam("user") Long userId) {
        return postService.fetchCurrentUserPost(userId);
    }
